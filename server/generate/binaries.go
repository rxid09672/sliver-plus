...
func SliverExecutable(name string, build *clientpb.ImplantBuild, config *clientpb.ImplantConfig, pbC2Implant *clientpb.HTTPC2ImplantConfig) (string, error) {
	...
	// *** BUILD DIVERSITY INTEGRATION (Wig Vector Space / Morpher) ***
	// Apply metamorphic transformations post-compilation
	if config.EnableBuildDiversity && config.DiversityConfig != nil {
		buildLog.Infof("Applying build diversity to %s", dest)
		err = ApplyBuildDiversity(dest, config, build)
		if err != nil {
			// Non-fatal: log warning and continue with original binary
			buildLog.Warnf("Build diversity failed, using original binary: %v", err)
		} else {
			buildLog.Infof("Build diversity applied successfully")
		}
	} else {
		buildLog.Debugf("Build diversity disabled for %s", dest)
	}
