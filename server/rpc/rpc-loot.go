...
// Recon - Run OSINT reconnaissance and store as loot
func (rpc *Server) Recon(ctx context.Context, req *clientpb.ReconReq) (*clientpb.ReconResult, error) {
	result, err := core.ReconTarget(req)
	if err != nil {
		return nil, err
	}
	
	// Publish event for new loot
	if result.LootID != "" {
		core.EventBroker.Publish(core.Event{
			EventType: consts.LootAddedEvent,
			Data:      []byte(result.LootID),
		})
	}
