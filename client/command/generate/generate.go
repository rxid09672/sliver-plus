...
func parseCompileFlags(cmd *cobra.Command, con *console.SliverClient) (string, *clientpb.ImplantConfig) {
	...
	// Build diversity flags
	diversity, _ := cmd.Flags().GetBool("diversity")
	novelTechniques, _ := cmd.Flags().GetBool("novel-techniques")
	seed, _ := cmd.Flags().GetString("seed")
	minNovelty, _ := cmd.Flags().GetFloat64("min-novelty")

	// Validate diversity flags
	if novelTechniques && !diversity {
		con.PrintErrorf("--novel-techniques requires --diversity to be enabled\n")
		return "", nil
	}
	if minNovelty < 0 || minNovelty > 1 {
	...
	if diversity {
		config.EnableBuildDiversity = true
		config.DiversityConfig = &clientpb.BuildDiversityConfig{
			EnableNovelTechniques: novelTechniques,
			ReproducibleSeed:      seed,
			MinNoveltyScore:       minNovelty,
			EvasionTechniqueCount: 5, // Default: apply 5 evasion techniques
			MinSGNIterations:      3, // Default: minimum SGN iterations
			MaxSGNIterations:      7, // Default: maximum SGN iterations
		}

		// Log diversity configuration for user awareness
		if novelTechniques {
			con.PrintInfof("Build diversity enabled with Wig Vector Space Metamorphism\n")
		} else {
			con.PrintInfof("Build diversity enabled with Morpher metamorphic engine\n")
		}
		if seed != "" {
