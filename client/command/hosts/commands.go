...
func Commands(con *console.SliverClient) []*cobra.Command {
	...
	hostsScanCmd := &cobra.Command{
		Use:   "scan <target> [target...]",
		Short: "Scan network targets and discover hosts",
		Long:  help.GetHelpFor([]string{consts.HostsStr, "scan"}),
		Run: func(cmd *cobra.Command, args []string) {
			HostsScanCmd(cmd, con, args)
		},
	}
	flags.Bind("hosts.scan", false, hostsScanCmd, func(f *pflag.FlagSet) {
		f.StringP("ports", "p", "", "port specification (e.g., '22,80,443' or '1-1000')")
		f.BoolP("service-detection", "s", false, "enable service version detection (slower)")
		f.BoolP("os-detection", "o", false, "enable OS detection (requires root/admin)")
		f.Int32("scan-timeout", 300, "scan timeout in seconds")
	})
	hostsCmd.AddCommand(hostsScanCmd)
