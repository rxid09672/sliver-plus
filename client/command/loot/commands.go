...
func Commands(con *console.SliverClient) []*cobra.Command {
	...
	lootReconCmd := &cobra.Command{
		Use:   "recon <type> <target>",
		Short: "Run OSINT reconnaissance and store as loot",
		Long:  help.GetHelpFor([]string{consts.LootStr, "recon"}),
		Run: func(cmd *cobra.Command, args []string) {
			LootReconCmd(cmd, con, args)
		},
		Args: cobra.MinimumNArgs(2),
	}
	lootCmd.AddCommand(lootReconCmd)
	flags.Bind("loot.recon", false, lootReconCmd, func(f *pflag.FlagSet) {
		f.Int32("timeout", 300, "reconnaissance timeout in seconds")
	})
	flags.BindFlagCompletions(lootReconCmd, func(comp *carapace.ActionMap) {
		(*comp)["timeout"] = carapace.ActionValues()
	})
	carapace.Gen(lootReconCmd).PositionalCompletion(
		carapace.ActionValues("domains", "emails", "all").Usage("Type of reconnaissance to perform"),
		carapace.ActionValues().Usage("Target domain (e.g., example.com)"),
	)
